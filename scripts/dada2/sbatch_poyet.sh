#!/bin/bash
#SBATCH -J dada2_poyet
#SBATCH -p normal
#SBATCH -n 1
#SBATCH -c 16
#SBATCH --mem-per-cpu=8G
#SBATCH -t 24:00:00
#SBATCH -o logs/dada2_poyet.%j.out
#SBATCH -e logs/dada2_poyet.%j.err
# ---------------------------------------------------------------------------
# DADA2 rerun -- Poyet et al. gut 16S (paired-end, truncLen 145/145)
#
#   mkdir -p logs && sbatch sbatch_poyet.sh
#
# Memory is set explicitly on purpose: Inti has no sane default, and mixing
# --mem with --mem-per-cpu is a fatal submission error.
# 16 x 8G = 128 GiB; pool=TRUE across many samples is the memory hog.
# ---------------------------------------------------------------------------
set -euo pipefail
...
STUDY_DIR="$DATA_ROOT/poyet_et_al"
...
srun --cpu-bind=none Rscript "$PROJECT/scripts/dada2/dada2_poyet.R"
