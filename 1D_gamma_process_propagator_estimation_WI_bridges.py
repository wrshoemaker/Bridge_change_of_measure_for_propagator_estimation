...
def gamma_distribution(x, shape, rate):
    """
    Evaluate the Gamma distribution at x with given
    ...
    shape : float
        Shape parameter α
    rate : float
        Rate parameter β (inverse scale)
...
if theta==0:
    ...
    target_prop = ...
# %% Make Lamperti transformation of data to use bridges of Wiener process and compute Jacobian of transformation
y0 = sqrt(x0) / 2.0 / D
yf = sqrt(xf) / 2.0 / D
J  = 1.0 / sqrt(xf) 
...
for i in range(Nbridges):
    ts_bridge, xs_bridge = Fill_gaps_with_Brownian_bridges(dt_bridge, tf, t0, y0, yf,D=1.0)

    # drift of gamma models
    b = -k * (xs_bridge - mu) * xs_bridge**(theta)
    ...
    prop_WI = propagator_WI_process(yf, tfmt0, y0, D)  # Wiener propagator
    Ls[i] = weight * prop_WI

empirical_prop = np.mean(Ls)*J
...
plt.ylim(0,1.2)
plt.savefig("figures/DE_propagator_MC_samples.pdf",bbox_inches="tight",transparent=True)
plt.show();plt.close()
...
plt.legend(fontsize=10,frameon=False,loc="upper right")
plt.savefig("figures/DE_propagator_MC_histogram.pdf",bbox_inches="tight",transparent=True)
plt.show();plt.close()
